...
namespace Celeste.Mod.SpeedrunTool.SaveLoad.Actions {
    public class DreamBlockAction : AbstractEntityAction {
        private Dictionary<EntityID, DreamBlock> _savedDreamBlocks = new Dictionary<EntityID, DreamBlock>();

        public override void OnQuickSave(Level level) {
            _savedDreamBlocks = level.Tracker.GetDictionary<DreamBlock>();
        }
        ...
        private void ResotreDreamBlockPosition(On.Celeste.DreamBlock.orig_ctor_EntityData_Vector2 orig,
            ...
            if (IsLoadStart && _savedDreamBlocks.ContainsKey(entityId)) {
                DreamBlock savedDreamBlock = _savedDreamBlocks[entityId];
                self.Position = savedDreamBlock.Position;
                Tween savedTween = savedDreamBlock.Get<Tween>();
                if (savedTween != null) {
                    self.Add(new Coroutine(RestorePosition(self, savedTween, data.Position + offset, data.FirstNodeNullable(offset).Value)));
                }
            }
        }
