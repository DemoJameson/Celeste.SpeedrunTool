...
namespace Celeste.Mod.SpeedrunTool.SaveLoad.Actions {
    public class TouchSwitchAction : AbstractEntityAction {
        private IEnumerable<TouchSwitch> _activatedTouchSwitches = Enumerable.Empty<TouchSwitch>();

        public override void OnQuickSave(Level level) {
            _activatedTouchSwitches = level.Tracker.GetCastEntities<TouchSwitch>()
        ...
        private void RestoreTouchSwitchState(On.Celeste.TouchSwitch.orig_ctor_Vector2 orig, TouchSwitch self,
            Vector2 position) {
            orig(self, position);
            TouchSwitch savedTouchSwitch = _activatedTouchSwitches.FirstOrDefault(touchSwitch => touchSwitch.Position == position);
        ...
        public override void OnClear() {
            _activatedTouchSwitches = Enumerable.Empty<TouchSwitch>();
        }
