...
namespace Celeste.Mod.SpeedrunTool.SaveLoad {
    internal class AutoLoadStateUtils {
        ...
        private static void AutoLoadStateWhenDeath(On.Celeste.AreaData.orig_DoScreenWipe orig, AreaData self,
            Scene scene,
            bool wipeIn, Action onComplete) {
            if (SpeedrunToolModule.Settings.Enabled && Manager.IsSaved &&
                !wipeIn && scene is Level level &&
                onComplete != null && (onComplete == level.Reload ||
                                       scene.Entities.FindFirst<PlayerDeadBody>()?.HasGolden == true)) {
